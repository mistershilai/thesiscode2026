...
We first study penicillin distribution across the Gaborone district, comprising CMS, Princess Marina Hospital, and downstream clinics and health posts, over a horizon of $T = 100$ periods with $\Gamma = 10$ and $\kappa = 10$.
...
\begin{figure}[htbp]
    ...
    \caption{Gaborone proof-of-concept: average unmet antimicrobial demand over the $T = 100$ period horizon (mean $\pm$ standard error across periods) under the three policies. \mynote{Periods are serially dependent through inventory carryover, so these intervals understate uncertainty; replace with across-replication standard errors once the Monte Carlo loop is added.} Adaptive recourse (ARO-ADR) achieves the lowest unmet demand.}
    \label{fig:gaborone}
\end{figure}
