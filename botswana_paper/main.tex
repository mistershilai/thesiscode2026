...
Retaining the min--max structure would be intractable at national scale, whereas the reformulated program is an ordinary mixed-integer program, which we solve with off-the-shelf solvers \citep{gurobi,mosek}.
...
The procurement premium of ARO-ADR over static robust is now 6.5\% and 6.4\%, against 0.9\% under illustrative costs, because the premium is a cost ratio and CMS's actual prices differ from the convenience value $c=1$.
...
Once $\Delta$ rises well above it, beyond roughly $10$ here, realized unmet demand saturates and becomes essentially invariant to the penalty.
