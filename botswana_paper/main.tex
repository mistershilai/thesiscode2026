...
Finally, the clinical cost of an antimicrobial stockout exceeds unit procurement cost by orders of magnitude \citep{poudel2023}, so the operationally relevant regime is one in which realized service is limited by physical network capacity rather than by the shortage penalty, and the recommended policy is insensitive to the precise penalty an agency assigns.
...
Section~EC.4 shows that realized service is unchanged for any $\Delta$ more than about ten times $c$, so what matters is that the ratio clears that threshold, not its exact value.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Economics of adaptive recourse in the Gaborone proof-of-concept. Transport cost is similar across policies (a), ARO-ADR avoids more shortage cost than it adds in procurement in every period (b), and ARO-ADR attains the lowest total objective (c).}
    \label{fig:gabcost}
\end{figure}
...
A stockout leaves an infection untreated or drives a switch to second-line therapy, and \citet{poudel2023} put the attributable cost of a resistant infection at US\$2{,}291 to US\$12{,}044 per case in upper-middle-income economies, Botswana's income group, against a unit procurement cost of a few currency units.
The ratio is therefore far above the threshold of roughly ten identified here, so the policy ordering holds and the recommendation does not depend on the exact penalty an agency assigns.
