...
Periods within a realization turn out to be close to independent here, with lag-one autocorrelation between $-0.13$ and $-0.02$, so a standard error taken across the periods of one path is a reasonable estimate of the spread of that path's average; averaging over $R$ independent realizations reduces the standard error by a further factor of roughly $\sqrt{R}$.
We report the latter because it is the uncertainty in the quantity the table and figure actually state.
