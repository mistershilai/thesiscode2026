...
A two-strain SEIR extension, in which treatment intensity depends on realized supply, preserves the policy ordering and leaves a lower resistant burden at every horizon.
\noindent
...
The framework is a deployable decision-support tool, and supply reliability acts on resistance as well as on coverage.
\end{abstract}
...
We observe survey counts $Y_{r,a,1},\dots,Y_{r,a,n}$, taken as independent draws from a negative-binomial law with mean $\mu_{r,a}$ and dispersion $\kappa>0$, and model the mean log-linearly, $\mu_{r,a} = \exp(x_{r,a}^{T}\beta)$, where $x_{r,a}$ collects age, infection-category, facility-tier, and drug-class indicators; regional variation enters through the admissions $N_{r,a}$ rather than through $\beta$.
...
A method-of-moments estimate over admitted patients gives $\kappa \approx 2.5$, indicating substantial overdispersion; the GLM's own fitted dispersion is not a stable alternative to this figure, which Section~EC.2 verifies directly.
Because it rests on one survey, we treat $\kappa$ as uncertain and sweep it in \Sec\ref{sec:results}.
The headline runs use $\kappa = 10$, a less variable regime than that estimate, and Figure~\ref{fig:kappa} shows the advantage of adaptivity grows as $\kappa$ falls, so this choice understates it.
...
In both scenarios the sorted service vectors dominate component by component, by at least $2.3$ percentage points from static robust to ARO-ADR and $10.9$ from deterministic to static robust.
...
CMS pays about $21.6$ BWP, near US\$1.60, per unit procured.
Even at a hundred units per case, $\Delta/c$ stays above the threshold of ten, so the recommendation does not turn on the penalty chosen.
...
Over two years the ordering is unchanged: ARO-ADR leaves 0.5\% of demand unmet on average, against 2.2\% for static robust and 10.4\% for deterministic (Table~\ref{tab:epi}).
At the epidemic peak its shortfall is 7.6\%, against 25.1\% for deterministic.
Because treatment depends on what arrives, ARO-ADR meets 99.9\% of the hospital's cephalosporin requirement against 93.6\% under deterministic planning, and ends the horizon with fewer infectious patients of both strains.
The effect on resistance is more subtle.
At two years ARO-ADR carries a lower \emph{absolute} resistant burden but a slightly higher resistant \emph{fraction}, because clearing susceptible infections faster shrinks the denominator.
That gap closes within three years, and over a 50-year horizon (Section~EC.3) ARO-ADR holds both the lower fraction and the lower burden.
Supply reliability therefore acts on resistance as well as coverage, though the calibration is illustrative and we do not read the magnitudes as forecasts.
...
\midrule
Deterministic & 10.35 & 63{,}365 & 4{,}740 \\
Static robust &  2.24 & 18{,}094 & 5{,}230 \\
ARO-ADR       &  0.52 &  8{,}912 & 5{,}317 \\
\bottomrule
\end{tabular}
\end{table}
...
The demand estimator is calibrated to one cross-sectional inpatient point-prevalence survey of ten hospitals spanning the four facility tiers, so the forecasts are best read as lower bounds on true facility need rather than complete demand; the optimization structure is unchanged by better inputs, but the numbers would move.
