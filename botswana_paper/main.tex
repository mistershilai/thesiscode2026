...
Second, the network is \emph{large and heterogeneous}: a cost-minimizing objective values unmet demand equally wherever it falls, so nothing prevents shortfalls settling on the districts most expensive to reach, and both tractability and the distribution of service must be designed in.
...
Finally, a stockout costs orders of magnitude more than a unit of procurement \citep{poudel2023}, so service is limited by network capacity rather than by the shortage penalty, and the recommendation is insensitive to that penalty.
...
Independent subproblems forbid shipments across district boundaries, so by construction no allocation can raise total service by moving supply between districts; each district is guaranteed its own allocation and transport whatever its size.
That guarantee is not an outcome criterion, so we do not report a $\mathrm{POF}$, which compares fairness-optimal allocations.
We instead evaluate the realized service vector against \eqref{eq:alphafair} in Figure~\ref{fig:equity}.
...
In 2025--26 the deterministic policy leaves North East, Ghanzi and Kgalagadi South at $29.6\%$, $27.2\%$ and $26.7\%$ unmet, and ARO-ADR brings the worst-served district to $4.6\%$: in the terms of \Sec\ref{sec:equity}, $\min_d s_d$ rises from $0.704$ to $0.954$.
Relative dispersion is unchanged, however, with a coefficient of variation across districts of $0.16$ under ARO-ADR against $0.18$ under deterministic and a Gini coefficient of $0.08$ against $0.10$.
The gain is a uniform improvement in level, not a reduction in inequality.
Figure~\ref{fig:equity} evaluates the allocation against the criterion of \Sec\ref{sec:equity}.
The sorted service vectors are ordered rank by rank, by at least $2.3$ and $11.0$ percentage points, so every welfare function increasing in district service ranks the policies identically and the choice does not require committing to an $\alpha$.
The margin widens with $\alpha$, from $18.4$ percentage points of equivalent service at $\alpha=0$ to $25.0$ under max-min, because the ARO-ADR service vector is nearly flat and the deterministic one is not.
...
We evaluate the resulting allocation against the $\alpha$-fairness family of \citet{bft12} and find the policy ranking invariant to $\alpha$, but we do not solve the fairness-optimal program.
The price of fairness of \citet{bft11}, and with it the position of our allocation on the efficiency-fairness frontier, is therefore still open, and characterizing it is the most immediate extension of this work.
...
Under the $\alpha$-fairness criterion the policy ranking is invariant to the equity weight, leaving the fairness-optimal allocation itself as the immediate next question.
