...
Each policy is evaluated over $R = 12$ independent demand realizations of $T = 100$ periods, drawn from the fitted negative-binomial model and shared across policies so the comparison is paired.
Averages are taken within a realization and standard errors across them; the resulting standard errors are two orders of magnitude smaller than the gaps between policies, so $R$ is not a binding constraint on what we conclude.
