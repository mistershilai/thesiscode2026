...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Realized unmet demand over two seasonal cycles. Supplying the seasonally-adjusted mean lets all policies pre-position ahead of peaks; ARO-ADR benefits most. Each curve is the mean over 12 independent demand realizations and the band is one standard error across replications, so the visible structure is not an artefact of a single path.}
    \label{fig:seasonal}
\end{figure}
