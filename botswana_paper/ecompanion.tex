...
\paragraph{The fitted dispersion is not stable, and we do not use it.}
\label{ec:kappa-stability}
Proposition~2 (consistency) concerns $\hat\beta_n$ and the resulting mean map $\hat D_r$, not the negative-binomial dispersion parameter $\hat\kappa$, which the same fit returns as a byproduct.
We verified that this byproduct is not trustworthy: refitting the identical survey table under three optimizer configurations (Nelder--Mead at 5{,}000 and 30{,}000 iterations, then BFGS) moves $\hat\kappa$ from $6.68$ to $8.02$ to $10.87$, with no change to the data or specification.
The mean structure is unaffected: the fitted demand $\hat D_r$ correlates at $0.98$ between the two most different fits.
So the dispersion parameter is not identified by this sample at this cell count, regardless of specification, while the demand estimate the model is built to produce is stable.
This is why Section~5 reports $\kappa$ from the method of moments rather than from the GLM, and it is checked automatically on every re-fit of the estimator, not asserted once.
...
Extending the integration to 50 years under each policy's steady coverage (Figure~\ref{fig:seirlong}) shows the resistant \emph{fraction} $I_r/(I_s+I_r)$ starting marginally higher under ARO-ADR and crossing below the deterministic path after about three years; at 50 years it is $71.9\%$ against $76.7\%$.
The absolute resistant burden $I_r$ is lower under ARO-ADR at every horizon.
The early gap is a denominator effect of faster susceptible-strain clearance, not evidence of greater resistance emergence, which is why we read the framework's benefits as coverage rather than resistance effects.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/seir_resistance_longhorizon}
    \caption{Two-strain SEIR extended to a 50-year horizon under each policy's steady supply coverage. \textbf{Left:} the resistant fraction $I_r/(I_s+I_r)$ starts marginally higher under better-supplied policies and crosses below after about three years. \textbf{Right:} absolute resistant burden $I_r$ is lower under ARO-ADR throughout.}
    \label{fig:seirlong}
\end{figure}

\paragraph{Sensitivity to the amplification rate.}
The crossover is not an artifact of $\rho = 0.07$.
Repeating the extension for $\rho$ from $0.01$ to $0.30$ moves it only between $2.8$ and $3.3$ years, and the absolute resistant burden stays lower under ARO-ADR at every horizon across that range (Figure~\ref{fig:rhosens}).
The long-horizon gap widens as $\rho$ falls, so the result is not driven by an aggressive amplification channel.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/seir_rho_sensitivity}
    \caption{Sensitivity of the long-horizon comparison to the amplification rate $\rho$; each curve is ARO-ADR minus deterministic. \textbf{Left:} the resistant fraction starts positive and crosses below zero between $2.8$ and $3.3$ years for every $\rho$. \textbf{Right:} the absolute resistant burden $I_r$ stays negative throughout, so ARO-ADR carries fewer resistant infections at every horizon. Coverage is held at the sequence each policy delivered.}
    \label{fig:rhosens}
\end{figure}
...
Below the procurement cost, serving demand is not cost-effective and unmet demand rises sharply; far above it, realized unmet demand saturates and is essentially invariant to the penalty, because achievable service is pinned by physical capacity and the uncertainty set.
...
\begin{figure}[H]
    ...
    \caption{Realized unmet demand (\%) across policies for varying shortage penalty (columns; unit procurement cost $1$) and dispersion $\kappa$ (rows). Far above the procurement cost, unmet demand saturates and the policy ordering is insensitive to the penalty.}
    \label{fig:penalty}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering\includegraphics[width=\textwidth]{figures/seasonal_unmet_no_multiplier}
        \caption{Stationary forecast supplied.}
    \end{subfigure}\hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering\includegraphics[width=\textwidth]{figures/seasonal_unmet_with_multiplier}
...
\paragraph{Minimum stock levels.}
CMS writes stock policy as a minimum cover, so we add an elastic floor requiring every facility to end each period holding at least $\beta$ periods of its nominal demand, applied equally to every facility, and sweep $\beta$ (Figure~\ref{fig:floor}).
Raising $\beta$ from $0$ to $2$ multiplies held inventory by $24.7$ while total cost falls slightly, because holding is cheap relative to the shortage penalty.
It buys little: deterministic unmet demand falls by $0.65$ percentage points, against $6.89$ points from switching to ARO-ADR at $29\%$ of the cost.
The stock rule an agency can write directly is dominated by the choice of planning model.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/equity_frontier}
    \caption{Elastic minimum stock cover $\beta$ swept across policies. \textbf{Left:} total cost over the horizon. \textbf{Right:} mean unmet demand. Movement along $\beta$ is small next to the gap between planning models.}
    \label{fig:floor}
\end{figure}
...
Kweneng is a persistent transport-cost outlier, carrying almost Greater Gaborone's volume over dispersed settlements at roughly five times its cost per unit shipped.
