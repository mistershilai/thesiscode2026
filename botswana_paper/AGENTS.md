...
- Use $a^T$ for transposes (write `^T` directly; do not define a `\tpose` macro).
...
1. **Abstract**: Declarative, short, to the point. State what you do, not background.
   ...
   - **Do not introduce an acronym in the abstract unless that acronym is reused later in the same abstract.** If "primal-dual hybrid gradient (PDHG)" appears once and never recurs in the abstract, drop the `(PDHG)`. The first body-text use of the term will reintroduce the acronym in the standard way.
